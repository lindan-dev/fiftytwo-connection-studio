...
const BetaSignup = () => {
    ...
    const fetchSignupCount = async () => {
      try {
        const {
          data,
          error
        } = await supabase.functions.invoke('get-beta-signups-count');
        if (!error && data) {
  ...
  return <section id="beta-signup" className="py-16 px-6">
      <div className="container max-w-2xl mx-auto">
        <div className="text-center mb-12">
          ...
          <div className="space-y-4 text-lg text-muted-foreground">
            ...
            {seatsLeft !== null && seatsLeft > 0 && <p className="text-xl font-bold text-primary">
                Only {seatsLeft} {seatsLeft === 1 ? 'seat' : 'seats'} left - sign up now!
              </p>}
            {seatsLeft === 0 && <p className="text-xl font-bold text-muted-foreground">
        ...
        <form onSubmit={handleSubmit} className="bg-card rounded-2xl p-8 shadow-card space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input id="partnerName" value={formData.partnerName} onChange={e => setFormData({
            ...formData,
            partnerName: e.target.value
          })} placeholder="Alex" />
          </div>

          <Button type="submit" size="lg" disabled={isSubmitting} className="w-full text-sm">
